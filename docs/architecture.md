...
`spectral_detection_service.py` adds Sentinel-2-compatible floating algae detection using B4 red, B8 NIR, and B11 SWIR reflectance grids. It calculates NDVI and FAI, applies lightweight thresholds, emits GeoJSON-compatible point features, and can convert detections into observation-like objects. Demo detection stays read-only. Controlled persistence is exposed through `POST /api/spectral/detect-and-ingest`, which accepts the same band payload as `/api/spectral/detect`, enforces a minimum confidence threshold, writes observations with `source_type=spectral_detection`, skips recent nearby spectral duplicates, and can create one derived patch from the accepted detections. The current implementation accepts local/mock band arrays and does not yet download Copernicus/Sentinel imagery directly.
